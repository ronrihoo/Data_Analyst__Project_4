...
# Load the Data
setwd("~/Data Analysis Projects/Project 4/Project4")
...
####Employment status shows to not affect the Prosper score; at least not signficantly. We could add a bit of noise to have a better look, and who knows, it might reveal something good, but there are other ways to look at this, so I'll move on for now.
...
ggplot(aes(IncomeRange, ProsperScore), 
       data = six_figures) +
  ggtitle('Prosper Score vs Income Range') +
  xlab("Income Range") +
  ylab("Prosper Score") +
  geom_boxplot() 
...
```{r echo=FALSE}
ggplot(aes(Occupation, ProsperScore),  
       data = data) +
  ggtitle('Prosper Score vs Occupation') + 
  xlab("Prosper Score") +
  ylab("Occupation") +
  geom_point() +
  # labels need to be turned at an angle to be readable
...
qplot(x = EmploymentStatus, y = CreditScoreRangeLower,
      data = subset(Unemployed, !is.na(CreditScoreRangeLower)),
      main = "Credit Score (Lower Range) vs Employment Status",
      xlab = "Employment Status",
      ylab = "Credit Score",
      geom = 'boxplot') 

qplot(x = EmploymentStatus, y = CreditScoreRangeUpper,
      data = subset(Unemployed, !is.na(CreditScoreRangeUpper)),
      main = "Credit Score (Upper Range) vs Employment Status",
      xlab = "Employment Status",
      ylab = "Credit Score",
      geom = 'boxplot') 
...
qplot(x = EmploymentStatus, y = ProsperScore,
      ...
      main = "Prosper Score vs Employment Status",
      xlab = "Employment Status",
      ylab = "Prosper Score",
      geom = 'boxplot') 
...
ggplot(aes(data.CreditScoreRangeLower, data.ProsperScore), 
       data = CS_PS) +
  ggtitle('Prosper Score vs Credit Score (Lower-Range)') +
  xlab("Credit Score") +
  ylab("Prosper Score") +
  geom_point() +
  scale_x_continuous(limits = c(600, 900))

ggplot(aes(data.CreditScoreRangeUpper, data.ProsperScore), 
       data = CS_PS) +
  ggtitle('Prosper Score vs Credit Score (Lower-Range)') +
  xlab("Credit Score") +
  ylab("Prosper Score") +
...
ggplot(aes(LoanOriginalAmount, BorrowerRate, color=factor(ProsperScore)),        
               data = data_n) +
  ggtitle('Relationship between Loan Amount, Borrower Rate, 
          and Prosper Score') + 
  geom_point(size = 2) +
  geom_point(alpha = 1/20) +
  geom_point(position = position_jitter(h = 0)) +
...
#### As apparent, the borrower rate decreases as the original loan amount increases. It's also evident that those with the higher Prosper scores are mostly awarded low rates.

------

# Reflection

#### This dataset shows that many factors go into assessing risk before a Prosper loan is granted, since the Prosper score seems to be affected by other variables that I have yet to analyze. But, ultimately, the variables that are most relative to the outcome of the borrower rate and loan amount are the individual's income range and Prosper score. Through analysis, it becomes apparent that the Prosper score also increases by about 2 points for those who have an income range of over six figures. So, overall, it's safe to conclude that the income range is the biggest factor in risk assessment for Prosper loans. However, other factors also affect the Prosper score, such as employment status and ones that I have not analyzed. In order to better understand the Prosper score, the rest of the 81 variables should be compared to it. My conclusion is that employed individuals with high income ranges are granted the lowest rates and the highest loan amounts. I suppose that, from the lenders' end, this goes hand-in-hand with the good ole market ideology of "low risk, low gain/loss; high risk, high gain/loss."
